...
class LvMetadataRW(object):
    ...
    def writelines(self, lines):
        ...
        try:
            misc.writeblockSUDO(self.metavol, self._offset, self._size, data, sudo=False)
        except se.MiscBlockWriteIncomplete:
            self.log.debug("write failed, retrying")
            misc.writeblockSUDO(self.metavol, self._offset, self._size, data, sudo=False)
...
class BlockStorageDomain(sd.StorageDomain):
    ...
    def invalidate(self):
        ...
        try:
            misc.dmRemoveMapping(self.sdUUID)
        except Exception, e:
            # log any other exception, but keep going
            self.log.error("Unexpected error", exc_info=True)
    ...
    def getDomainPath(cls, sdUUID):
        ...
        Return domain's path or ERROR for non-block domains
        """
        vg = lvm.getVG(sdUUID)
        if _isSD(vg):
            return os.path.join(cls.mountpoint, sdUUID)
        else:
    ...
    def getAllImages(self):
        ...
        for lv in lvs:
            tags.update(lv.tags)
        # Drop non IU_ tags and strip "IU_" prefix
    ...
    def doUnmountMaster(cls, masterdir):
        ...
        if fileUtils.isMounted(mountPoint=masterdir):
            ...
            if rc or fileUtils.isMounted(mountPoint=masterdir):
...
def getBlockStorageDomainList():
    ...
    #FIXME : Should be a simple os.listdir()
    stale = glob.glob(os.path.join(BlockStorageDomain.mountpoint, "*"))
    try:
        # Now get the actual BlockSD list
        vgl = [v for v in lvm.getAllVGs() if _isSD(v)]
    except Exception, e:
        log.warning("Can't get VG list.", exc_info=True)
    ...
    for v in vgl:
        try:
            dl.append(BlockStorageDomain(v.name))
        except Exception, e:
            log.warning("Can't create domain from VG=%s", v.name, exc_info=True)
    ...
    try:
        ...
        for i in stale:
            fileUtils.cleanupdir(i)
    except Exception, e:
        log.warning("Can't remove stale leftovers", exc_info=True)
