...
class SPM:
    ...
    def __releaseLocks(cls):
        ...
        try:
            ...
            cls.log.error("__releaseLocks failed")
    ...
    def start(self, spUUID, prevID, prevLVER, recoveryMode, scsiFencing, maxHostID, expectedDomVersion=None):
        ...
        :type spUUID: UUID
        :param prevID: The ID of the Previous owner of the SPM.
                       Required only during recovery from failed SPM Host. Use -1 if not recovering.
        :type prevID: int
        :param prevLVER: The Lock version of the previous owner. Use -1 if not recovering from a failed host.
        :type prevLVER: int
        ...
        :raises: :exc:`storage_exception.SpmParamsMismatch` if *prevID* or *prevLVER* have incorrect values.
        """
        self.log.debug("spUUID=%s", spUUID)
        self.lock.acquire()
        try:
            ...
            # FIXME: fence is assigned but never used
            fence = misc.parseBool(scsiFencing)
    ...
    def _stop(self, spUUID):
        ...
        with rmanager.acquireResource(STORAGE, "upgrade_" + spUUID, LockType.exclusive):
            ...
            def cancelUpgrade(self, sdUUID, req, res):
                try:
                    self._domainsToUpgrade.remove(sdUUID)
                except ValueError:
                    pass
