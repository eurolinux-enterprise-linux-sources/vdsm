...
class GeneralException(Exception):
    ...
    def response(self):
        ...
        if type(args) != types.ListType:
            args = [args]
        return dict(status=dict(code=self.code, message=str(self)), args=args)
...
class ImagesNotSupportedError(StorageException):
    code = 329
    message = "This domain deos not support images"
...
class StorageServerValidationError(StorageException):
    code = 453
    message = "Error storage server validation"
...
class StorageServerAccessPermissionError(StorageException):
    code = 469
    message = "Host Agent doesn't have enough access permissions mount point"
