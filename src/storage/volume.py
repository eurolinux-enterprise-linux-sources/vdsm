...
def QEMU_IMG():
    return config.get('vars', 'qemuimg')
...
class Volume:
    ...
    def rebaseVolumeRollback(cls, taskObj, sdUUID, srcImg, srcVol, dstFormat, srcParent, unsafe):
        ...
        """
        cls.log.info("rebaseVolumeRollback: sdUUID=%s srcImg=%s srcVol=%s "\
    ...
    def rebase(self, backingVol, backingVolPath, backingFormat, unsafe, rollback):
        ...
        (rc, out, err) = qemuRebase(self.getVolumePath(), self.getFormat(), backingVolPath,
                                    backingFormat, unsafe, self.idle, vars.task.aborting)
        if rc:
    ...
    def teardown(self, justme=False, setrw=False):
        """
        Teardown volume. If justme is false, the entire COW chain is teared down.
        """
        self.log.info("Volume: tearing down volume %s/%s justme %s" % (self.sdUUID, self.volUUID, justme))
        self.llTeardown(rw=self.isLeaf(), setrw=setrw)
        if justme:
            return True
        pvol = self.getParentVolume()
        if pvol:
            pvol.teardown(justme=False, setrw=setrw)
        return True
...
def createVolume(parent, parent_format, volume, size, format, prealloc):
    ...
    cmd = [QEMU_IMG(), "create", "-f", fmt2str(format)]
    cwd = None
...
def qemuCommit(src, fmt, idle, stop):
    ...
    cmd = NICE + [QEMU_IMG(), "commit", "-f", fmt2str(fmt), src]
...
def qemuRebase(src, srcFormat, backingFile, backingFormat, unsafe, idle, stop):
    """
    ...
    cmd = NICE + [QEMU_IMG(), "rebase", "-f", srcFormat, "-b", backingFile, "-F", backingFormat]
    if unsafe:
        cmd += ["-u"]
    cmd += [src]
...
def qemuConvert(src, dst, src_fmt, dst_fmt, idle, stop, size, dstvolType):
    ...
    if src_fmt == "raw" and dst_fmt == "raw" and dstvolType == type2name(PREALLOCATED_VOL):
        (rc, out, err) = misc.ddWatchCopy(src=src, dst=dst, stop=stop, idle=idle, size=size)
    else:
        cmd = NICE + [QEMU_IMG(), "convert", "-f", src_fmt, src, "-O", dst_fmt, dst]
